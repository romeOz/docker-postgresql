...
docker run --name base_4 -d --link base_1:base_1 -e 'PG_MODE=slave' -e 'PG_TRUST_LOCALNET=true' -e 'REPLICATION_HOST=base_1' -e 'PG_RESTORE=default'  -v $(pwd)/vol94/backup_master:/tmp/backup psg-9.4; sleep 10
...
docker run --name base_1 -d -e 'PG_MODE=master' -e 'PG_RESTORE=default'  -v $(pwd)/vol94/backup_master:/tmp/backup psg-9.4; sleep 10
...
docker run --name base_4 -d --link base_1:base_1 -e 'PG_MODE=slave' -e 'PG_TRUST_LOCALNET=true' -e 'REPLICATION_HOST=base_1' -e 'PG_RESTORE=default'  -v $(pwd)/vol94/backup_slave:/tmp/backup psg-9.4; sleep 10
docker exec -it base_1 sudo -u postgres psql test_1 -c "INSERT INTO foo (name) VALUES ('Jack');"; sleep 5
...
docker run --name base_1 -d -e 'PG_MODE=master' -e 'PG_RESTORE=default'  -v $(pwd)/vol94/backup_slave:/tmp/backup psg-9.4; sleep 10
docker run --name base_2 -d --link base_1:base_1 -e 'PG_MODE=slave' -e 'PG_TRUST_LOCALNET=true' -e 'REPLICATION_HOST=base_1' psg-9.4; sleep 10
docker exec -it base_1 sudo -u postgres psql test_1 -c "INSERT INTO foo (name) VALUES ('Tom');"; sleep 5
...
echo ""
echo "-- Restore master"
docker run --name master -d --link slave_2:slave_2 -e 'PG_MODE=master_restore' -e 'PG_TRUST_LOCALNET=true' -e 'REPLICATION_HOST=slave_2' psg-9.4; sleep 10
...
echo ""
echo "-- Restore slave from backup-master"
docker run --name base_4 -d --link base_1:base_1 -e 'PG_MODE=slave' -e 'PG_TRUST_LOCALNET=true' -e 'REPLICATION_HOST=base_1' -e 'PG_RESTORE=default'  -v $(pwd)/vol93/backup_master:/tmp/backup psg-9.3; sleep 10
docker exec -it base_1 sudo -u postgres psql test_1 -c "INSERT INTO foo (name) VALUES ('Bob');"; sleep 5
docker exec -it base_3 bash -c 'sudo -u postgres psql test_1 -c "SELECT * FROM foo;"  | grep -c -w "Bob"'
docker exec -it base_4 bash -c 'sudo -u postgres psql test_1 -c "SELECT COUNT(*) FROM foo;" | grep -c -w "3"'
echo ""
...
echo ""
echo "-- Restore master from backup-master"
docker run --name base_1 -d -e 'PG_MODE=master' -e 'PG_RESTORE=default'  -v $(pwd)/vol93/backup_master:/tmp/backup psg-9.3; sleep 10
...
echo ""
echo "-- Restore slave from backup-slave"
docker run --name base_4 -d --link base_1:base_1 -e 'PG_MODE=slave' -e 'PG_TRUST_LOCALNET=true' -e 'REPLICATION_HOST=base_1' -e 'PG_RESTORE=default'  -v $(pwd)/vol93/backup_slave:/tmp/backup psg-9.3; sleep 10
docker exec -it base_1 sudo -u postgres psql test_1 -c "INSERT INTO foo (name) VALUES ('Jack');"; sleep 5
docker exec -it base_4 bash -c 'sudo -u postgres psql test_1 -c "SELECT * FROM foo;"  | grep -c -w "Chip"'
docker exec -it base_4 bash -c 'sudo -u postgres psql test_1 -c "SELECT * FROM foo;"  | grep -c -w "Jack"'
docker exec -it base_4 bash -c 'sudo -u postgres psql test_1 -c "SELECT COUNT(*) FROM foo;" | grep -c -w "3"'
...
echo ""
echo "-- Restore master from backup-slave"
docker run --name base_1 -d -e 'PG_MODE=master' -e 'PG_RESTORE=default'  -v $(pwd)/vol93/backup_slave:/tmp/backup psg-9.3; sleep 10
docker run --name base_2 -d --link base_1:base_1 -e 'PG_MODE=slave' -e 'PG_TRUST_LOCALNET=true' -e 'REPLICATION_HOST=base_1' psg-9.3; sleep 10
docker exec -it base_1 sudo -u postgres psql test_1 -c "INSERT INTO foo (name) VALUES ('Tom');"; sleep 5
docker exec -it base_2 bash -c 'sudo -u postgres psql test_1 -c "SELECT * FROM foo;"  | grep -c -w "Tom"'
docker exec -it base_2 bash -c 'sudo -u postgres psql test_1 -c "SELECT * FROM foo;"  | grep -c -w "Chip"'
...
echo ""
echo "-- Restore master"
docker run --name master -d --link slave_2:slave_2 -e 'PG_MODE=master_restore' -e 'PG_TRUST_LOCALNET=true' -e 'REPLICATION_HOST=slave_2' psg-9.3; sleep 10
