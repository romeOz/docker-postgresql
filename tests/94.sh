...
docker run --name base_1 -d --net pg_test psg-9.4; sleep 5
docker run --name base_2 -d --net pg_test psg-9.4; sleep 10
...
docker run --name base_1 -d --net pg_test -e 'PG_MODE=master' -e 'DB_NAME=db_1,test_1' psg-9.4; sleep 10
echo "-- Backup"
docker run -it --rm --net pg_test -e 'PG_MODE=backup' -e 'REPLICATION_HOST=base_1' -e 'REPLICATION_PORT=5432' -v $(pwd)/vol94/backup:/tmp/backup psg-9.4 | grep -wc 'backup completed'; sleep 10
echo "-- Check"
...
echo
echo "-- Create slave"
docker run --name base_2 -d --net pg_test -e 'PG_MODE=slave' -e 'PG_TRUST_LOCALNET=true' -e 'REPLICATION_HOST=base_1' -e 'REPLICATION_USER=replica' -e 'REPLICATION_PASS=replica' psg-9.4; sleep 10
docker run --name base_3 -d --net pg_test -e 'PG_MODE=slave' -e 'PG_TRUST_LOCALNET=true' -e 'REPLICATION_HOST=base_1' psg-9.4; sleep 10
...
echo
echo "-- Backup"
docker run -it --rm --net pg_test -e 'PG_MODE=backup' -e 'REPLICATION_HOST=base_1' -e 'REPLICATION_PORT=5432' -v $(pwd)/vol94/backup_master:/tmp/backup psg-9.4 | grep -wc 'backup completed'; sleep 10
echo
echo "-- Restore slave from backup-master"
docker run --name base_4 -d --net pg_test -e 'PG_MODE=slave' -e 'PG_TRUST_LOCALNET=true' -e 'REPLICATION_HOST=base_1' -e 'PG_RESTORE=default'  -v $(pwd)/vol94/backup_master:/tmp/backup psg-9.4; sleep 10
...
docker run --name base_1 -d --net pg_test -e 'PG_MODE=master' -e 'PG_RESTORE=default'  -v $(pwd)/vol94/backup_master:/tmp/backup psg-9.4; sleep 10
docker run --name base_2 -d --net pg_test -e 'PG_MODE=slave' -e 'PG_TRUST_LOCALNET=true' -e 'REPLICATION_HOST=base_1' psg-9.4; sleep 10
...
docker run --name base_3 -d --net pg_test -e 'PG_MODE=slave_wal' -e 'PG_TRUST_LOCALNET=true' -e 'REPLICATION_HOST=base_1' psg-9.4; sleep 10

echo
echo "-- Backup"
docker run -it --rm --net pg_test -e 'PG_MODE=backup' -e 'REPLICATION_HOST=base_3' -e 'REPLICATION_PORT=5432' -v $(pwd)/vol94/backup_slave:/tmp/backup psg-9.4 | grep -wc 'backup completed'; sleep 10
echo
echo "-- Restore slave from backup-slave"
docker run --name base_4 -d --net pg_test -e 'PG_MODE=slave' -e 'PG_TRUST_LOCALNET=true' -e 'REPLICATION_HOST=base_1' -e 'PG_RESTORE=default'  -v $(pwd)/vol94/backup_slave:/tmp/backup psg-9.4; sleep 10
docker exec -it base_1 sudo -u postgres psql test_1 -c "INSERT INTO foo (name) VALUES ('Jack');"; sleep 5
...
docker run --name base_1 -d --net pg_test -e 'PG_MODE=master' -e 'PG_RESTORE=default'  -v $(pwd)/vol94/backup_slave:/tmp/backup psg-9.4; sleep 10
docker run --name base_2 -d --net pg_test -e 'PG_MODE=slave' -e 'PG_TRUST_LOCALNET=true' -e 'REPLICATION_HOST=base_1' psg-9.4; sleep 10
docker exec -it base_1 sudo -u postgres psql test_1 -c "INSERT INTO foo (name) VALUES ('Tom');"; sleep 5
...
echo
echo "-- Create master"
docker run --name master -d --net pg_test -e 'PG_MODE=master' -e 'DB_NAME=db_1,test_1' psg-9.4; sleep 10
echo
echo "-- Create slaves"
docker run --name slave_1 -d --net pg_test -e 'PG_MODE=slave' -e 'PG_TRUST_LOCALNET=true' -e 'REPLICATION_HOST=master' psg-9.4; sleep 10
docker run --name slave_2 -d --net pg_test -e 'PG_MODE=slave_wal' -e 'PG_TRUST_LOCALNET=true' -e 'REPLICATION_HOST=master' psg-9.4; sleep 10
...
echo
echo "-- Restore master"
docker run --name master -d --net pg_test -e 'PG_MODE=master_restore' -e 'PG_TRUST_LOCALNET=true' -e 'REPLICATION_HOST=slave_2' psg-9.4; sleep 10
