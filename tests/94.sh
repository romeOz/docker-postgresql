...
echo "-- Backup"
docker run -it --rm --net pg_test -e 'PG_MODE=backup' -e 'REPLICATION_HOST=base_1' -e 'REPLICATION_PORT=5432' -v ${DIR_VOLUME}/backup:/tmp/backup psg-9.4 | grep -wc 'backup completed'; sleep 10
echo "-- Check"
docker run -it --rm -e 'PG_CHECK=default' -e 'DB_NAME=db_1' -v ${DIR_VOLUME}/backup:/tmp/backup psg-9.4 | tail -n 1 | grep -c 'Success'; sleep 5
docker run -it --rm -e 'PG_CHECK=/tmp/backup/backup.last.tar.bz2' -e 'DB_NAME=test_1' -v ${DIR_VOLUME}/backup:/tmp/backup psg-9.4 | tail -n 1 | grep -c 'Success'; sleep 5
docker run -it --rm -e 'PG_CHECK=default' -e 'DB_NAME=db' -v ${DIR_VOLUME}/backup:/tmp/backup psg-9.4 | tail -n 1 | grep -c 'Fail'; sleep 5
echo
echo "-- Clear"
...
docker run --name base_4 -d --net pg_test -e 'PG_MODE=slave' -e 'PG_TRUST_LOCALNET=true' -e 'REPLICATION_HOST=base_1' -e 'PG_RESTORE=default'  -v ${DIR_VOLUME}/backup:/tmp/backup psg-9.4; sleep 10
...
docker run --name base_1 -d --net pg_test -e 'PG_MODE=master' -e 'PG_RESTORE=default'  -v ${DIR_VOLUME}/backup:/tmp/backup psg-9.4; sleep 10
...
echo "-- Backup"
mkdir -p ${DIR_VOLUME}/backup
docker run -it --rm --net pg_test -e 'PG_MODE=backup' -e 'REPLICATION_HOST=base_3' -e 'REPLICATION_PORT=5432' -v ${DIR_VOLUME}/backup:/tmp/backup psg-9.4 | grep -wc 'backup completed'; sleep 10
echo
