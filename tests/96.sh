...
echo "-- Building PostgreSQL 9.6 image"
docker build -t psg-9.6 ../9.6/
docker network create pg_test
DIR_VOLUME=$(pwd)/vol96
mkdir -p ${DIR_VOLUME}/backup

echo
echo "-- Testing PostgreSQL 9.6 is running"
docker run --name base_1 -d --net pg_test psg-9.6; sleep 5
docker run --name base_2 -d --net pg_test psg-9.6; sleep 10
...
echo
echo "-- Testing backup/checking on PostgreSQL 9.6"
docker run --name base_1 -d --net pg_test -e 'PG_MODE=master' -e 'DB_NAME=db_1,test_1' psg-9.6; sleep 10
echo "-- Backup"
docker run -it --rm --net pg_test -e 'PG_MODE=backup' -e 'REPLICATION_HOST=base_1' -e 'REPLICATION_PORT=5432' -v ${DIR_VOLUME}/backup:/tmp/backup psg-9.6 | grep -wc 'backup completed'; sleep 10
echo "-- Check"
docker run -it --rm -e 'PG_CHECK=default' -e 'DB_NAME=db_1' -v ${DIR_VOLUME}/backup:/tmp/backup psg-9.6 | tail -n 1 | grep -c 'Success'; sleep 5
docker run -it --rm -e 'PG_CHECK=/tmp/backup/backup.last.tar.bz2' -e 'DB_NAME=test_1' -v ${DIR_VOLUME}/backup:/tmp/backup psg-9.6 | tail -n 1 | grep -c 'Success'; sleep 5
docker run -it --rm -e 'PG_CHECK=default' -e 'DB_NAME=db' -v ${DIR_VOLUME}/backup:/tmp/backup psg-9.6 | tail -n 1 | grep -c 'Fail'; sleep 5
echo
echo "-- Clear"
...
echo
echo
echo "-- Testing master/slave on PostgreSQL 9.6"
echo
echo "-- Create master"
docker run --name base_1 -d --net pg_test -e 'PG_MODE=master' -e 'DB_NAME=db_1,test_1' psg-9.6; sleep 10
...
echo
echo "-- Create slave"
docker run --name base_2 -d --net pg_test -e 'PG_MODE=slave' -e 'PG_TRUST_LOCALNET=true' -e 'REPLICATION_HOST=base_1' -e 'REPLICATION_USER=replica' -e 'REPLICATION_PASS=replica' psg-9.6; sleep 10
docker run --name base_3 -d --net pg_test -e 'PG_MODE=slave' -e 'PG_TRUST_LOCALNET=true' -e 'REPLICATION_HOST=base_1' psg-9.6; sleep 10
...
docker run --name base_4 -d --net pg_test -e 'PG_MODE=slave' -e 'PG_TRUST_LOCALNET=true' -e 'REPLICATION_HOST=base_1' -e 'PG_RESTORE=default'  -v ${DIR_VOLUME}/backup:/tmp/backup psg-9.6; sleep 10
...
docker run --name base_1 -d --net pg_test -e 'PG_MODE=master' -e 'PG_RESTORE=default'  -v ${DIR_VOLUME}/backup:/tmp/backup psg-9.6; sleep 10
docker run --name base_2 -d --net pg_test -e 'PG_MODE=slave' -e 'PG_TRUST_LOCALNET=true' -e 'REPLICATION_HOST=base_1' psg-9.6; sleep 10
...
docker run --name base_3 -d --net pg_test -e 'PG_MODE=slave_wal' -e 'PG_TRUST_LOCALNET=true' -e 'REPLICATION_HOST=base_1' psg-9.6; sleep 10

echo
echo "-- Backup"
mkdir -p ${DIR_VOLUME}/backup
docker run -it --rm --net pg_test -e 'PG_MODE=backup' -e 'REPLICATION_HOST=base_3' -e 'REPLICATION_PORT=5432' -v ${DIR_VOLUME}/backup:/tmp/backup psg-9.6 | grep -wc 'backup completed'; sleep 10
echo
...
echo
echo
echo "-- Testing failover master on PostgreSQL 9.6"
echo
echo "-- Create master"
mkdir -p ${DIR_VOLUME}/backup
docker run --name master -d --net pg_test -e 'PG_MODE=master' -e 'DB_NAME=db_1,test_1' psg-9.6; sleep 10
echo
echo "-- Create slaves"
docker run --name slave_1 -d --net pg_test -e 'PG_MODE=slave' -e 'PG_TRUST_LOCALNET=true' -e 'REPLICATION_HOST=master' psg-9.6; sleep 10
docker run --name slave_2 -d --net pg_test -e 'PG_MODE=slave_wal' -e 'PG_TRUST_LOCALNET=true' -e 'REPLICATION_HOST=master' psg-9.6; sleep 10
...
echo
echo "-- Restore master"
docker run --name master -d --net pg_test -e 'PG_MODE=master_restore' -e 'PG_TRUST_LOCALNET=true' -e 'REPLICATION_HOST=slave_2' psg-9.6; sleep 10
