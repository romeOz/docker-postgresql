...
DIR_VOLUME=$(pwd)/vol95
mkdir -p ${DIR_VOLUME}/backup

echo
echo "-- Testing PostgreSQL 9.5 is running"
docker run --name base_1 -d --net pg_test psg-9.5; sleep 5
docker run --name base_2 -d --net pg_test psg-9.5; sleep 10
...
echo
echo "-- Create slave"
docker run --name base_2 -d --net pg_test -e 'PG_MODE=slave' -e 'PG_TRUST_LOCALNET=true' -e 'REPLICATION_HOST=base_1' -e 'REPLICATION_USER=replica' -e 'REPLICATION_PASS=replica' psg-9.5; sleep 10
docker run --name base_3 -d --net pg_test -e 'PG_MODE=slave' -e 'PG_TRUST_LOCALNET=true' -e 'REPLICATION_HOST=base_1' psg-9.5; sleep 10
...
docker run --name base_3 -d --net pg_test -e 'PG_MODE=slave_wal' -e 'PG_TRUST_LOCALNET=true' -e 'REPLICATION_HOST=base_1' psg-9.5; sleep 10
...
echo
echo "-- Create slaves"
docker run --name slave_1 -d --net pg_test -e 'PG_MODE=slave' -e 'PG_TRUST_LOCALNET=true' -e 'REPLICATION_HOST=master' psg-9.5; sleep 10
docker run --name slave_2 -d --net pg_test -e 'PG_MODE=slave_wal' -e 'PG_TRUST_LOCALNET=true' -e 'REPLICATION_HOST=master' psg-9.5; sleep 10
...
echo
echo "-- Restore master"
docker run --name master -d --net pg_test -e 'PG_MODE=master_restore' -e 'PG_TRUST_LOCALNET=true' -e 'REPLICATION_HOST=slave_2' psg-9.5; sleep 10
